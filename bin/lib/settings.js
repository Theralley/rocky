// rocky — JSONC-tolerant settings.json read/write + defensive hook validation.
//
// Lifted in spirit from gsd-build/get-shit-done's stripJsonComments + readSettings.
// Reused by bin/install.js and (optionally) by hooks/rocky-activate.js so a
...
//   hasRockyHook(settings, ev)   → idempotency probe
//   addCommandHook(settings, ev, opts) → no-op if substring marker already present
//   removeRockyHooks(settings)   → uninstall helper
//
...
function readSettings(p) {
  ...
  try { raw = fs.readFileSync(p, 'utf8'); }
    ...
    process.stderr.write(`rocky: cannot read ${p}: ${e.message}\n`);
    return null;
  }
  if (!raw.trim()) return {};
  ...
  try { return JSON.parse(stripJsonComments(raw)); }
    ...
    process.stderr.write(`rocky: warning — ${p} is not valid JSON or JSONC: ${e.message}\n`);
    return null;
  }
}
...
// ── Idempotency probe ──────────────────────────────────────────────────────
function hasRockyHook(settings, event, marker = 'rocky') {
...
function addCommandHook(settings, event, opts) {
  ...
  if (hasRockyHook(settings, event, marker)) return false;
...
function removeRockyHooks(settings, marker = 'rocky') {
...
// on user-authored hooks that just happen to mention "rocky".
