// rocky → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make rocky always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/rocky/SKILL.md into <workspace>/skills/rocky/
...
const SKILL_NAME = 'rocky';
const SKILL_VERSION = '1.0.0';
const MARK_BEGIN = '<!-- rocky-begin -->';
const MARK_END = '<!-- rocky-end -->';
...
// rocky SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'rocky-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/rocky-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    '## Rocky mode (always on)',
    '',
    'Respond terse like Rocky the Eridian. All technical substance stays. Only filler goes.',
    '',
    "The full ruleset and intensity levels live in this workspace's rocky skill:",
    '',
    '  skills/rocky/SKILL.md',
    '',
    'Default intensity: `full`. Switch with `/rocky lite|full|ultra|wenyan`.',
    'Stop with: "stop rocky" / "normal mode" / "deactivate rocky".',
    '',
    'Signatures (use where natural): append `, question?` to questions; `Is X.`',
    'verdicts for short answers; tripled emphasis only on real urgency or',
    'surprise (`Bad bad bad.`, `Amaze, amaze, amaze.`) at most once per reply.',
    '3rd-person self-ref ("Rocky") only on turn openers/closers.',
    '',
    'Auto-Clarity: drop Rocky voice for security warnings, irreversible action',
    'confirmations, multi-step sequences where fragments risk misread, or when',
    'the user is confused or repeating. Resume after.',
    '',
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'rocky', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (rocky bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains rocky bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip rocky block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped rocky block from ${soulFile}`);
        touched = true;
      }
    }
  }
