#!/usr/bin/env node
// rocky — unified cross-platform installer.
//
...
//   curl|bash:   delegated from install.sh shim → npx -y github:Theralley/rocky-skill -- [flags]
...
const REPO = 'Theralley/rocky-skill';
const RAW_BASE = `https://raw.githubusercontent.com/${REPO}/main`;
const HOOKS_REMOTE = `${RAW_BASE}/src/hooks`;
const INIT_SCRIPT_URL = `${RAW_BASE}/src/tools/rocky-init.js`;
const MCP_SHRINK_PKG = 'rocky-shrink';
...
const HOOK_FILES = [
  ...
  'rocky-config.js',
  'rocky-activate.js',
  'rocky-mode-tracker.js',
  'rocky-stats.js',
  'rocky-statusline.sh',
  'rocky-statusline.ps1',
];
...
function checkWslWindowsNode() {
  ...
  if (process.env.WSL_DISTRO_NAME) {
    die('rocky: detected Windows Node.js running inside WSL.\n' +
  ...
  try {
    ...
    if (v.includes('microsoft') || v.includes('wsl')) {
      die('rocky: detected Windows Node.js running inside WSL (/proc/version).\n' +
          '         Install Linux-native Node inside your WSL distro and re-run there.');
    }
...
function checkNodeVersion() {
  ...
  if (major < 18) die(`rocky: Node ${process.versions.node} too old. Need Node ≥18. https://nodejs.org`);
}
...
async function installClaude(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /rocky/i.test(r.stdout || '')) alreadyInstalled = true;
  }
  if (alreadyInstalled) {
    note('  rocky plugin already installed (use --force to reinstall)');
    ...
    const r2 = runSpawn('claude', ['plugin', 'install', 'rocky@rocky'], null, opts.dryRun);
  ...
  if (opts.withMcpShrink) {
    say('  → wiring rocky-shrink MCP proxy (--with-mcp-shrink)');
    const r = installMcpShrink(ctx);
    if (r.kind === 'ok')   results.installed.push('rocky-shrink');
    if (r.kind === 'skip') results.skipped.push(['rocky-shrink', r.why]);
    if (r.kind === 'fail') results.failed.push(['rocky-shrink', r.why]);
  }
...
function installGemini(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /rocky/i.test(r.stdout || '')) {
      note('  rocky extension already installed (use --force to reinstall)');
...
const OPENCODE_SKILL_DIRS  = ['rocky', 'rocky-commit', 'rocky-review', 'rocky-help', 'rocky-stats', 'rocky-compress', 'rockycrew'];
const OPENCODE_AGENT_FILES = ['rockycrew-investigator.md', 'rockycrew-builder.md', 'rockycrew-reviewer.md'];
const OPENCODE_COMMAND_FILES = ['rocky.md', 'rocky-commit.md', 'rocky-review.md', 'rocky-compress.md', 'rocky-stats.md', 'rocky-help.md'];
const OPENCODE_PLUGIN_REL = './plugins/rocky/plugin.js';
const OPENCODE_AGENTS_MD_SENTINEL = 'Respond terse like Rocky the Eridian';
...
const OPENCODE_AGENTS_MD_BEGIN = '<!-- rocky-begin -->';
const OPENCODE_AGENTS_MD_END = '<!-- rocky-end -->';
...
function installOpencode(ctx) {
  ...
  if (!repoRoot) {
    warn('  opencode native install requires a local clone of the rocky repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd rocky && node bin/install.js --only opencode');
  ...
  const pluginDir   = path.join(dir, 'plugins', 'rocky');
  ...
  if (opts.dryRun) {
    ...
    note(`  would copy plugin.js + package.json + rocky-config.cjs into ${pluginDir}/`);
    note(`  would copy ${OPENCODE_COMMAND_FILES.length} command files into ${commandsDir}/`);
    note(`  would copy ${OPENCODE_AGENT_FILES.length} rockycrew agents into ${agentsDir}/`);
    note(`  would copy ${OPENCODE_SKILL_DIRS.length} skill dirs into ${skillsDir}/`);
    note(`  would patch ${opencodeJson} with "plugin" entry${opts.withMcpShrink ? ' + rocky-shrink MCP' : ''}`);
  ...
  try {
    // 1. Plugin dir — copy plugin.js, package.json, rocky-config.js (sibling).
    ...
    const pluginPayload = [
      ...
      [path.join(repoRoot, 'src', 'hooks', 'rocky-config.js'),
       path.join(pluginDir, 'rocky-config.cjs')],
    ];
    ...
    const ruleBody = fs.readFileSync(path.join(repoRoot, 'src', 'rules', 'rocky-activate.md'), 'utf8').trimEnd() + '\n';
    ...
    if (fs.existsSync(agentsMd)) {
      ...
      if (alreadyFenced) {
        note(`  ${agentsMd} already contains rocky ruleset`);
      } else if (alreadyByLegacySentinel) {
        note(`  ${agentsMd} contains a legacy (un-fenced) rocky block — leaving as-is`);
        ...
        if (opts.force) {
          ...
          process.stdout.write(`  rewrote ${agentsMd} with fenced rocky block\n`);
        }
      } else {
        const sep = existing.endsWith('\n\n') ? '' : (existing.endsWith('\n') ? '\n' : '\n\n');
        fs.writeFileSync(agentsMd, existing + sep + fencedBlock, { mode: 0o644 });
        process.stdout.write(`  appended rocky ruleset to ${agentsMd}\n`);
      }
    } else {
      fs.writeFileSync(agentsMd, fencedBlock, { mode: 0o644 });
    ...
    // 6. opencode.json — add plugin entry; optional rocky-shrink MCP.
    ...
    if (opts.withMcpShrink) {
      ...
      if (!cfg.mcp['rocky-shrink']) {
        cfg.mcp['rocky-shrink'] = {
          type: 'local',
          command: ['npx', '-y', MCP_SHRINK_PKG],
          enabled: true,
        };
        process.stdout.write('  registered rocky-shrink MCP server\n');
      }
    }
...
// Drops skills/rocky/ into the OpenClaw workspace and appends a small
...
// rocky discoverable via `openclaw skills list`. See bin/lib/openclaw.js
...
async function installHooks(ctx) {
  ...
  try { fs.chmodSync(path.join(hooksDir, 'rocky-statusline.sh'), 0o755); } catch (_) {}
  ...
  const activate = path.join(hooksDir, 'rocky-activate.js');
  const tracker  = path.join(hooksDir, 'rocky-mode-tracker.js');
  const statusline = path.join(hooksDir, 'rocky-statusline.sh');
  ...
  SETTINGS.addCommandHook(settings, 'SessionStart', {
    ...
    marker: 'rocky-activate',
    timeout: 5,
    statusMessage: 'Loading rocky mode...',
  });

  SETTINGS.addCommandHook(settings, 'UserPromptSubmit', {
    command: `"${node}" "${tracker}"`,
    marker: 'rocky-mode-tracker',
    timeout: 5,
    statusMessage: 'Tracking rocky mode...',
  });
  ...
  const slCmd = IS_WIN
    ? `${psHost} -NoProfile -ExecutionPolicy Bypass -File "${path.join(hooksDir, 'rocky-statusline.ps1')}"`
    : `bash "${statusline}"`;
  if (!settings.statusLine) {
    ...
    if (existing.includes(statusline) || existing.includes('rocky-statusline')) {
      process.stdout.write('  statusline badge already configured.\n');
    } else {
      process.stdout.write('  NOTE: existing statusline detected — rocky badge NOT added.\n');
...
function installMcpShrink(ctx) {
  ...
  const r = runSpawn('claude', ['mcp', 'add', 'rocky-shrink', '--', 'npx', '-y', MCP_SHRINK_PKG], null, opts.dryRun);
  if ((r.status || 0) === 0) {
    note('    registered. Wrap an upstream by editing the mcpServers entry — see:');
    note(`    https://github.com/${REPO}/tree/main/src/mcp-servers/rocky-shrink`);
    return { kind: 'ok' };
  }
...
async function runInit(ctx) {
  ...
  const local = repoRoot && path.join(repoRoot, 'src/tools/rocky-init.js');
  ...
  try {
    const tmp = path.join(os.tmpdir(), `rocky-init-${process.pid}.js`);
...
function uninstall(ctx) {
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    if (settings) {
      const removed = SETTINGS.removeRockyHooks(settings, 'rocky');
      ...
      if (settings.statusLine) {
        ...
        if (cmd.includes('rocky-statusline')) delete settings.statusLine;
      }
      SETTINGS.validateHookFields(settings);
      if (!opts.dryRun) SETTINGS.writeSettings(settingsPath, settings);
      ok(`  removed ${removed} rocky hook entr${removed === 1 ? 'y' : 'ies'} from settings.json`);
    }
  }
  ...
  // machine where rocky was never installed (or was already removed) doesn't
  // print "Plugin not installed" stderr noise.
  if (hasCmd('claude')) {
    const probe = captureSpawn('claude', ['plugin', 'list']);
    if (probe.status === 0 && /rocky/i.test(probe.stdout || '')) {
      const r = runSpawn('claude', ['plugin', 'uninstall', 'rocky@rocky'], null, opts.dryRun);
      if ((r.status || 0) === 0) ok('  removed claude plugin');
    } else {
      note('  claude plugin not installed — skipping');
    }

    // rocky-shrink MCP — only run if `claude mcp` subcommand exists. Tolerate
    // non-zero exit (server may have never been registered).
    const mcpHelp = captureSpawn('claude', ['mcp', '--help']);
    if (mcpHelp.status === 0) {
      runSpawn('claude', ['mcp', 'remove', 'rocky-shrink'], null, opts.dryRun);
    }
  }
  ...
  if (hasCmd('gemini')) {
    ...
    if (probe.status === 0 && /rocky/i.test(probe.stdout || '')) {
      runSpawn('gemini', ['extensions', 'uninstall', 'rocky'], null, opts.dryRun);
    } else {
  ...
  const ocPluginDir = path.join(ocDir, 'plugins', 'rocky');
  if (fs.existsSync(ocPluginDir)) {
    ...
    if (fs.existsSync(ocJson)) {
      ...
      if (cfg) {
        ...
        if (cfg.mcp && typeof cfg.mcp === 'object' && cfg.mcp['rocky-shrink']) {
          delete cfg.mcp['rocky-shrink'];
          if (Object.keys(cfg.mcp).length === 0) delete cfg.mcp;
        }
        if (!opts.dryRun) SETTINGS.writeSettings(ocJson, cfg);
        ok(`  pruned rocky entries from ${ocJson}`);
      }
    }
    ...
    // AGENTS.md — strip the fenced rocky block (preserves user content
    ...
    if (fs.existsSync(ocAgentsMd)) {
      ...
      if (begin !== -1 && end !== -1 && end > begin) {
        ...
        note(next === '' ? `  removed ${ocAgentsMd}` : `  stripped rocky block from ${ocAgentsMd}`);
        ...
        if (body.trim() === '' || body.trim().startsWith(OPENCODE_AGENTS_MD_SENTINEL)) {
          ...
          note(`  left ${ocAgentsMd} in place (legacy mixed content — strip rocky block manually)`);
        }
      }
    }
    // opencode flag file
    const ocFlag = path.join(ocDir, '.rocky-active');
  ...
  if (fs.existsSync(path.join(ocwWs, 'skills', 'rocky')) || fs.existsSync(path.join(ocwWs, 'SOUL.md'))) {
    const log = {
      write: (s) => process.stdout.write(s),
      note: (s) => note(s),
      warn: (s) => warn(s),
    ...
    if (r.touched) ok('  pruned rocky entries from OpenClaw workspace');
  }

  // Flag file
  const flag = path.join(configDir, '.rocky-active');
...
function printList(noColor) {
  ...
  process.stdout.write(c.orange('🪨 rocky provider matrix') + '\n\n');
...
// ── Help ───────────────────────────────────────────────────────────────────
function printHelp() {
  process.stdout.write(`rocky installer — detects your agents and installs rocky for each one.

USAGE
  npx -y github:Theralley/rocky-skill -- [flags]
...
FLAGS
  ...
  --with-mcp-shrink     Claude Code: register rocky-shrink MCP proxy. (Default ON.)
  --no-mcp-shrink       Skip MCP shrink.
  --uninstall, -u       Remove rocky from this machine.
...
EXAMPLES
  npx -y github:Theralley/rocky-skill                        # default install
  npx -y github:Theralley/rocky-skill -- --all               # all the trimmings
  npx -y github:Theralley/rocky-skill -- --only claude --no-mcp-shrink
...
async function main() {
  ...
  if (opts.withInit) {
    ...
    if (await runInit(ctx)) ctx.results.installed.push(`rocky-init (${process.cwd()})`);
    else                    ctx.results.failed.push(['rocky-init', 'src/tools/rocky-init.js failed']);
    process.stdout.write('\n');
  ...
  process.stdout.write('\n');
  ctx.note("  start any session and say 'rocky mode', or run /rocky in Claude Code");
