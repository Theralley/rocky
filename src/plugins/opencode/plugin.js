...
// security-hardened helpers from rocky-config.js via createRequire so the
// symlink-safe flag-write code lives in one place.
//
// Layout once installed:
//   ~/.config/opencode/plugins/rocky/
//   ├── package.json
//   ├── plugin.js              ← this file
//   └── rocky-config.js      ← copied sibling of src/hooks/rocky-config.js
//
// Always-on rocky ruleset is provided separately via
...
// When installed: rocky-config.cjs sits next to plugin.js (copied by
...
// to the canonical src/hooks/rocky-config.js, which lives in a directory
// whose own package.json pins "type": "commonjs". One source of truth
// either way.
function loadConfig() {
  try { return require(join(here, 'rocky-config.cjs')); }
  catch (_) { return require(join(here, '..', '..', 'hooks', 'rocky-config.js')); }
}
const config = loadConfig();

const { getDefaultMode, safeWriteFlag, readFlag, VALID_MODES } = config;

// Modes handled by independent skills — not selectable via /rocky <arg>.
...
const flagPath = path.join(opencodeConfigDir(), '.rocky-active');

function reinforcementLine(mode) {
  return 'ROCKY MODE ACTIVE (' + mode + '). ' +
...
// null when the prompt doesn't change state. Mirrors rocky-mode-tracker.js.
function parseModeChange(promptRaw) {
  ...
  // like rocky" doesn't trip the activation regex.
  if (/\b(stop|disable|deactivate|turn off)\b.*\brocky\b/i.test(prompt) ||
      /\brocky\b.*\b(stop|disable|deactivate|turn off)\b/i.test(prompt) ||
      /\bnormal mode\b/i.test(prompt)) {
    return 'off';
  }

  // Natural-language activation
  if (/\b(activate|enable|turn on|start|talk like)\b.*\brocky\b/i.test(prompt) ||
      /\brocky\b.*\b(mode|activate|enable|turn on|start)\b/i.test(prompt)) {
  ...
  if (prompt.startsWith('/rocky')) {
    const parts = prompt.split(/\s+/);
    const cmd = parts[0];
    const arg = parts[1] || '';

    if (cmd === '/rocky-commit')   return 'commit';
    if (cmd === '/rocky-review')   return 'review';
    if (cmd === '/rocky-compress') return 'compress';
...
export const RockyPlugin = async (_ctx) => ({
  ...
  // rocky is active so the model can't drift mid-session. Returning an
