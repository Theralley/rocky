#!/usr/bin/env node
// rocky-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/rocky-stats.js
// Inside Claude:   /rocky-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag } = require('./rocky-config');
...
// Detect *.original.md / *.md pairs left behind by rocky-compress. The
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nRocky Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /rocky-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '🪨 rocky armed but no turns yet — rocky.sh';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `🪨 Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — rocky.sh`;
  }
  return `🪨 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — rocky.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    savings = `Est. without rocky:  ${estNormal.toLocaleString()}\n` +
    ...
    savings = 'Rocky not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.rocky-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`rocky-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('rocky-stats: no Claude Code session found.\n');
    process.exit(1);
  }

  const parsed = parseSession(sessionFile);
  const mode = readFlag(path.join(claudeDir, '.rocky-active'));

  // Append a snapshot of this session's totals to the lifetime log. Multiple
  // /rocky-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /rocky-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .rocky-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `🪨 ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.rocky-statusline-suffix'), suffix);
  }
