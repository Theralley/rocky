...
FLAG_FILE="$CLAUDE_DIR/.rocky-active"

HOOK_FILES=("package.json" "rocky-config.js" "rocky-activate.js" "rocky-mode-tracker.js" "rocky-stats.js" "rocky-statusline.sh")

# Detect if rocky is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/rocky*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Rocky appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable rocky"
  echo ""
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the rocky SessionStart and UserPromptSubmit"
    ...
    ROCKY_SETTINGS="$SETTINGS" ROCKY_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references rocky
...
echo ""
echo "Other agents:"
echo "  npx skills remove rocky    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable rocky  # Claude Code plugin"
echo "  gemini extensions uninstall rocky  # Gemini CLI"
