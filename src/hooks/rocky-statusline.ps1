...
$Flag = Join-Path $ClaudeDir ".rocky-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[ROCKY]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[ROCKY:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via ROCKY_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by rocky-stats.js. Refuses reparse
# points and strips control bytes (matches statusline.sh hardening). Until
# /rocky-stats has run at least once, the suffix file is absent and nothing
# is rendered — safe default for fresh installs.
if ($env:ROCKY_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $ClaudeDir ".rocky-statusline-suffix"
