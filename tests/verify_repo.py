#!/usr/bin/env python3
"""Local verification runner for rocky install surfaces."""
...
def verify_skill_frontmatter_upload_compatibility() -> None:
    ...
    skill_paths = [
        ROOT / "skills/rocky/SKILL.md",
        ROOT / "skills/rocky-commit/SKILL.md",
        ROOT / "skills/rocky-help/SKILL.md",
        ROOT / "skills/rocky-review/SKILL.md",
        ROOT / "skills/rocky-compress/SKILL.md",
    ]
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/rocky/SKILL.md"

    skill_copies = [
        ROOT / "plugins/rocky/skills/rocky/SKILL.md",
    ]
    ...
    with zipfile.ZipFile(ROOT / "dist" / "rocky.skill") as archive:
        ensure("rocky/SKILL.md" in archive.namelist(), "rocky.skill missing rocky/SKILL.md")
        ensure(
            archive.read("rocky/SKILL.md").decode("utf-8")
            == skill_source.read_text(encoding="utf-8"),
            "rocky.skill payload mismatch",
        )

    ensure(
        (ROOT / "bin" / "install.js").exists(),
        "bin/install.js missing — package.json bin entry would break npx rocky",
    )
    ensure(
    ...
    print("Synced copies, rocky.skill zip, and installer entrypoints OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/rocky/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "src/hooks/rocky-config.js"])
    run(["node", "--check", "src/hooks/rocky-activate.js"])
    run(["node", "--check", "src/hooks/rocky-mode-tracker.js"])
    ...
    run(["bash", "-n", "src/hooks/rocky-statusline.sh"])

    # Ensure install/uninstall scripts include rocky-config.js
    ...
    ensure("rocky-config.js" in install_sh, "install.sh missing rocky-config.js")
    ensure("rocky-config.js" in uninstall_sh, "uninstall.sh missing rocky-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "src/hooks/rocky-statusline.ps1").read_text(encoding="utf-8")

    ensure("rocky-config.js" in install_text, "install.ps1 missing rocky-config.js")
    ensure("rocky-config.js" in uninstall_text, "uninstall.ps1 missing rocky-config.js")
    ensure("rocky-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("rocky-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[ROCKY" in statusline_text, "rocky-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "skills/rocky-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/rocky-compress").glob("*.original.md"))
    ensure(fixtures, "No rocky-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} rocky-compress fixture pairs")
...
def verify_compress_cli() -> None:
    ...
    skip_result = run(
        ...
        cwd=ROOT / "skills/rocky-compress",
        check=False,
    )
...
def verify_hook_install_flow() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rocky-verify-") as temp_root:
        ...
        activate = run(
            ["node", "src/hooks/rocky-activate.js"],
            env=hook_env,
        )
        ensure("ROCKY MODE ACTIVE" in activate.stdout, "activation output missing rocky banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".rocky-active").read_text(encoding="utf-8") == "full", "activation flag should default to full")

        # Test configurable default mode via ROCKY_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "src/hooks/rocky-activate.js"],
            env={**hook_env, "ROCKY_DEFAULT_MODE": "ultra"},
        )
        ensure("ROCKY MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".rocky-active").read_text(encoding="utf-8") == "ultra",
            "ROCKY_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "src/hooks/rocky-activate.js"],
            env={**hook_env, "ROCKY_DEFAULT_MODE": "off"},
        )
        ensure("ROCKY MODE ACTIVE" not in activate_off.stdout, "off mode should not emit rocky banner")
        ensure(not (claude_dir / ".rocky-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /rocky when default is off — should NOT write flag
        subprocess.run(
            ["node", "src/hooks/rocky-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "ROCKY_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/rocky"}',
            capture_output=True,
        ...
        ensure(not (claude_dir / ".rocky-active").exists(), "/rocky with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".rocky-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/rocky ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "ROCKY MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".rocky-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".rocky-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".rocky-active").write_text("wenyan-ultra")
        statusline = run(
            ["bash", "src/hooks/rocky-statusline.sh"],
            env=hook_env,
        )
        ensure("[ROCKY:WENYAN-ULTRA]" in statusline.stdout, "statusline badge output mismatch")
        ...
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-rocky settings")
        ensure(not (claude_dir / ".rocky-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="rocky-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "src/hooks/rocky-activate.js"], env=hook_env)
