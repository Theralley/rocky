...
test('addCommandHook is idempotent on substring marker', () => {
  const s = {};
  const a = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/abs/path/rocky-activate.js', marker: 'rocky-activate' });
  const b = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/different/abs/path/rocky-activate.js', marker: 'rocky-activate' });
  assert.equal(a, true);
  assert.equal(b, false);
  assert.equal(s.hooks.SessionStart.length, 1);
});

test('hasRockyHook detects via substring', () => {
  const s = { hooks: { SessionStart: [{ hooks: [{ type: 'command', command: 'node /x/rocky-activate.js' }] }] } };
  assert.equal(SETTINGS.hasRockyHook(s, 'SessionStart', 'rocky-activate'), true);
  assert.equal(SETTINGS.hasRockyHook(s, 'SessionStart', 'gsd'), false);
  assert.equal(SETTINGS.hasRockyHook(s, 'UserPromptSubmit'), false);
});

test('removeRockyHooks tolerates malformed hook event values without throwing', () => {
  ...
  let removed;
  assert.doesNotThrow(() => { removed = SETTINGS.removeRockyHooks(s, 'rocky'); });
  assert.equal(removed, 0);
  assert.equal(s.hooks, undefined);
});

test('removeRockyHooks strips by marker and cleans empties', () => {
  const s = {
    hooks: {
      SessionStart: [
        { hooks: [{ type: 'command', command: 'rocky-x' }] },
        { hooks: [{ type: 'command', command: 'other' }] },
      ],
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'rocky-y' }] }],
    },
  };
  const removed = SETTINGS.removeRockyHooks(s, 'rocky');
...
test('rewriteLegacyManagedHookCommands rewrites bare-node managed scripts', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node /abs/hooks/rocky-activate.js' },
  ...
  assert.equal(n, 1);
  assert.match(s.hooks.SessionStart[0].hooks[0].command, /"\/usr\/local\/bin\/node" "\/abs\/hooks\/rocky-activate\.js"/);
...
test('rewriteLegacyManagedHookCommands ignores already-absolute node commands', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/usr/local/bin/node" "/abs/hooks/rocky-activate.js"' },
      ] }],
    },
  };
