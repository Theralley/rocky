...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'rocky-freshinstall-'));
}
...
function rockyHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'rocky-activate.js')),     'rocky-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'rocky-mode-tracker.js')), 'rocky-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'rocky-config.js')),       'rocky-config.js missing');
    ...
    assert.ok(SETTINGS.hasRockyHook(settings, 'SessionStart', 'rocky-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasRockyHook(settings, 'UserPromptSubmit', 'rocky-mode-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /rocky-statusline/,
      'statusLine command does not reference rocky');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = rockyHookCommands(settings, 'SessionStart', 'rocky-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart rocky hook, got ${sessStart.length}`);

    const ups = rockyHookCommands(settings, 'UserPromptSubmit', 'rocky-mode-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit rocky hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips rocky hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['rocky-activate.js', 'rocky-mode-tracker.js', 'rocky-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining rocky-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /rocky/, `${ev} still has rocky hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = rockyHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at rocky should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /rocky-statusline/,
      'rocky statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    // Rocky hooks must be wired.
    assert.ok(SETTINGS.hasRockyHook(parsed, 'SessionStart', 'rocky-activate'));
    assert.ok(SETTINGS.hasRockyHook(parsed, 'UserPromptSubmit', 'rocky-mode-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'rocky', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'rocky', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- rocky-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- rocky-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /Respond terse like Rocky the Eridian/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'rocky', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- rocky-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    ...
    assert.match(soulRaw, /<!-- rocky-begin -->/, 'rocky block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'rocky')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- rocky-begin -->/, 'rocky block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- rocky-end -->/, 'rocky end marker survived uninstall');
...
test('rocky-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'rocky-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `rocky-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'rocky', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /Respond terse like Rocky the Eridian/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/rocky-activate.js"',
      marker: 'rocky-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/rocky-activate.js"',
      marker: 'rocky-activate',
    });
