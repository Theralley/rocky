#!/usr/bin/env python3
"""Benchmark rocky vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "rocky" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, rocky_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("rocky", rocky_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        rocky_medians = statistics.median(
            [t["output_tokens"] for t in entry["rocky"]]
        )
        savings = 1 - (rocky_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "rocky_median": int(rocky_medians),
    ...
    avg_rocky = round(statistics.mean([r["rocky_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Rocky (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['rocky_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_rocky']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark rocky vs normal Claude")
    ...
    rocky_system = load_rocky_system()
    ...
    results = run_benchmarks(client, args.model, prompts, rocky_system, args.trials)
