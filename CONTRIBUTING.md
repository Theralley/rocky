...
Thanks for considering a contribution. Rocky is a multi-agent skill that
makes 30+ AI coding agents talk in compressed rocky-style prose. Most
contributions fall into one of three buckets:

1. **Editing skill prose** — change how rocky speaks, what intensity levels do, what slash commands trigger.
...
Rocky like simple. Small focused PR > big rewrite.

---

## Quick orientation

The repo distributes one skill (rocky) plus a handful of sub-skills
(rocky-commit, rocky-review, rocky-compress, rockycrew-*) to many
...
copies live under `plugins/rocky/` and similar mirror dirs — those are
...
|---|---|
| Rocky behavior (intensity levels, voice, rules) | `skills/rocky/SKILL.md` |
| Rocky commit-message format | `skills/rocky-commit/SKILL.md` |
| Rocky code-review format | `skills/rocky-review/SKILL.md` |
| Rocky compress logic | `skills/rocky-compress/SKILL.md` and `skills/rocky-compress/scripts/` |
| Rocky quick-reference card | `skills/rocky-help/SKILL.md` |
| Rockycrew decision guide (when to delegate to subagents) | `skills/rockycrew/SKILL.md` |
| rockycrew subagent definitions | `agents/rockycrew-investigator.md`, `agents/rockycrew-builder.md`, `agents/rockycrew-reviewer.md` |
| Auto-activation rule body (Cursor/Windsurf/Cline/Copilot) | `src/rules/rocky-activate.md` |
| Add support for a new agent | `bin/install.js` (PROVIDERS array) |
| Per-repo init script (drops rule files into a user's repo) | `src/tools/rocky-init.js` |
| Claude Code hooks | `src/hooks/rocky-activate.js`, `src/hooks/rocky-mode-tracker.js`, `src/hooks/rocky-config.js`, `src/hooks/rocky-statusline.sh`, `src/hooks/rocky-statusline.ps1` |
| Settings.json read/write helpers | `bin/lib/settings.js` |
| MCP shrink server | `src/mcp-servers/rocky-shrink/` |
...
| `dist/rocky.skill` | ZIP of `skills/rocky/` (gitignored; rebuilt by CI on each push to `main`) |

`rocky-commit`, `rocky-review`, `rocky-help`, and `rocky-stats` are **not** mirrored under `plugins/rocky/skills/` by CI. Claude Code reaches them through the standalone hook + skill install path and `npx skills` carries them to other agents. If you see `plugins/rocky/skills/rocky-stats/` checked in, treat it as a legacy hand-committed copy — the workflow in `.github/workflows/sync-skill.yml` does not touch it.
...
4. If the skill should be in the Claude Code plugin, add a sync step to `.github/workflows/sync-skill.yml` so CI mirrors it into `plugins/rocky/skills/<name>/`.
...
- **Conventional Commits** for the commit subject. See `skills/rocky-commit/SKILL.md` for the format we use here.
...
- **Mention the CI sync.** If you edited a source-of-truth file, note it: "CI will resync `plugins/rocky/skills/...` on merge."

PR descriptions don't need to be long. Rocky style fine. Just say what change, why.
...
- **Hooks must silent-fail on filesystem errors.** A `try/catch` that swallows the error is correct here. A hook that throws blocks Claude Code session start — that's user-facing breakage. See existing patterns in `src/hooks/rocky-activate.js`.
...
- **Symlink-safe flag writes via `safeWriteFlag()`** in `src/hooks/rocky-config.js`. The flag file lives at a predictable path under `$CLAUDE_CONFIG_DIR/`; without `O_NOFOLLOW` and a parent-symlink check, a local attacker can clobber any file the user can write.
...
Rocky like contribution. You bring rock, rocky put rock in pile. Pile
